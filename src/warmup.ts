...
export function startWarmup(agent: AriaAgent): Promise<string> {
  if (!warmupPromise) {
...
async function runWarmup(agent: AriaAgent): Promise<string> {
  const cwd = agentCwd();
  ...
  const text =
    greeting?.trim() ||
    "ARIA online — work desk ready. What are we tackling? (Home tasks go to Amelia.)";
  emitGreeting(text);
