...
export async function cancelStaleRuns(
  ...
  try {
    ...
    for (const run of items) {
      ...
      console.error(`[aria-agent] Cancelling stale run ${run.id}`);
    ...
    console.error(`[aria-agent] Could not list/cancel stale runs: ${msg}`);
  }
  ...
  if (agentId.startsWith("bc-")) {
    try {
      ...
      if (info.status === "running") {
        console.error(
          `[aria-agent] Cloud agent ${agentId} still marked running; waiting…`,
        );
...
export async function withAgentBusyRecovery<T>(
  ...
  for (let attempt = 1; attempt <= maxAttempts; attempt++) {
    try {
      ...
      console.error(
        `[aria-agent] Session busy (attempt ${attempt}/${maxAttempts}); recovering…`,
      );
