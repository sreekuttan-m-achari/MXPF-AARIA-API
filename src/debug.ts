...
export function isDebugEnabled(): boolean {
  return truthyEnv("AARIA_DEBUG");
}

export function isStreamDebugEnabled(): boolean {
  return isDebugEnabled() && truthyEnv("AARIA_DEBUG_STREAM");
}

function logFilePath(): string | undefined {
  const override = process.env.AARIA_DEBUG_LOG?.trim();
  if (override) return resolve(override);
  if (isDebugEnabled()) {
    return resolve(process.cwd(), ".aria-conversations.ndjson");
  }
  return undefined;
}
...
export function logConversation(entry: ConversationLogEntry): void {
  ...
  const prefix = `[aria-debug] ${entry.transport} id=${entry.id}`;
...
export function logStreamChunk(id: string, text: string): void {
  ...
  console.error(`[aria-debug] ws chunk id=${id}: ${JSON.stringify(text)}`);
}

export function logDebugStartup(): void {
  if (!isDebugEnabled()) return;
  const file = logFilePath();
  console.error("[aria-debug] conversation logging enabled");
  if (file) {
    console.error(`[aria-debug] writing to ${file}`);
  }
  if (isStreamDebugEnabled()) {
    console.error("[aria-debug] stream chunk logging enabled");
  }
}
